...
function JournalCard({ journal, onClick }) {
  const dateStr = new Date(journal.createdAt).toLocaleDateString('ru-RU', {
    ...
    <div className="card card--interactive fade-in" onClick={onClick} style={{ cursor: 'pointer' }}>
      <div className="card__header">
        <div>
          ...
          <div className="card__meta">Создан: {dateStr}</div>
        </div>
      ...
      <div style={{ display:'flex', gap:10, flexWrap:'wrap', marginTop:8 }}>
        ...
        <span className="chip chip-yellow">👥 {journal.studentCount} уч.</span>
        <span className="chip" style={{ background:'var(--bg3)', color:'var(--text3)' }}>
          📅 {journal.columnCount} дней
        </span>
      </div>
...
export default function HomePage() {
  ...
  if (loading) return (
    <div className="loader-wrap">
      ...
      <span style={{ color:'var(--text3)' }}>Загрузка журналов...</span>
    </div>
  );
  ...
  return (
    <div className="page-wrap fade-in">
      <div className="page-header">
        <div>
          <h1 className="page-title">Мои журналы</h1>
          <p className="page-subtitle">
            {user?.groupName
              ? `Группа ${user?.groupName} · ${filtered.length} журналов`
              : `${filtered.length} журналов в системе`}
          </p>
        </div>
        {user?.isAdmin && (
      ...
      <div className="grid" style={{ gridTemplateColumns:'repeat(auto-fill,minmax(180px,1fr))', marginBottom:28 }}>
        <div className="stat-card">
          ...
          <div className="stat-card__label">Всего журналов</div>
        </div>
        <div className="stat-card">
          <div className="stat-card__value">
            {[...new Set(journals.map(j => j.subjectName).filter(Boolean))].length}
          </div>
          <div className="stat-card__label">Предметов</div>
        </div>
        <div className="stat-card">
          <div className="stat-card__value">
      ...
      {filtered.length === 0 ? (
        <div className="empty">
          ...
          <div className="empty__text">
            {search ? 'Ничего не найдено по вашему запросу' : 'Журналов пока нет'}
          </div>
          {user?.isAdmin && !search && (
